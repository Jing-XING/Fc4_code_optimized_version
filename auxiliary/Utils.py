...
def color1_to_color3(a):
    return torch.cat(((a[0] + a[1])/2, (a[2] + a[3])/2, (a[4] + a[5])/2), 1)
...
def cal_degree_loss(label_ori,r,g,b):
    '''input the original label and the classify model for colorconstancy
    out put the degree loss'''
    output=torch.cat([r,g,b],dim=0)
    pre = output.argmax(dim=1,keepdim=True)
    return get_angular_loss(pre.float().view(1,3), label_ori)
